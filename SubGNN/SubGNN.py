...
from datasets import SubgraphDataset, SubgraphPairDataset
...
class SubGNN(pl.LightningModule):
    ...
    def __init__(self, hparams: Dict, graph_path: str, subgraph_path: str,
        ...
        self.device_num = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        if self.hparams["use_neighborhood"]:
            ...
            for l in range(self.hparams["n_layers"]):
                ...
                if "batch_norm" in self.hparams and self.hparams["batch_norm"]:
                    curr_layer["batch_norm"] = nn.BatchNorm1d(self.hparams["node_embed_size"]).to(self.device_num)
                    curr_layer["batch_norm_out"] = nn.BatchNorm1d(self.hparams["node_embed_size"]).to(self.device_num)
        ...
        # >>> TODO modify final layer for our purposes
        #self.lin3 =  nn.Linear(self.hparams["linear_hidden_dim_2"], self.num_classes)
        # should be hidden * 2 since concatenate drug and disease
        self.lin3 =  nn.Linear(self.hparams["linear_hidden_dim_2"] * 2, self.num_classes)
        ...
        if "ff_attn" in self.hparams and self.hparams["ff_attn"]:
            self.attn_vector = torch.nn.Parameter(torch.zeros((hid_dim,1), dtype=torch.float).to(self.device_num), requires_grad=True)
...
# forward pass
    ...
    def run_mpn_layer(self, dataset_type, mpn_fn, subgraph_ids, subgraph_idx, cc_ids, \
        ...
        anchor_patches, anchor_mask, anchor_embeds = get_anchor_patches(dataset_type, self.hparams, \
            ...
            self.anchors_pos_ext, self.anchors_structure, layer_num, channel, inside, self.device_num)
    ...
    def forward(self, dataset_type, N_I_cc_embed, N_B_cc_embed, \
        ...
        # >>> do not apply final layer here
        # only output after concatenating drug and disease
        # = run this forward pass twice
...
##################################################
 # training, val, test steps
    ...
    def _get_batch(self, batch):
        ...
        return subgraph_ids, cc_ids, subgraph_idx, NP_sim, I_S_sim, B_S_sim
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        # forward pass x2
        drugs, diseases = batch["drugs"], batch["diseases"]
        features_drugs = self.forward("train", self.train_N_I_cc_embed, self.train_N_B_cc_embed, \
            self.train_S_I_cc_embed, self.train_S_B_cc_embed, self.train_P_I_cc_embed, self.train_P_B_cc_embed, \
            *self._get_batch(drugs))
        features_diseases = self.forward("train", self.train_N_I_cc_embed, self.train_N_B_cc_embed, \
            self.train_S_I_cc_embed, self.train_S_B_cc_embed, self.train_P_I_cc_embed, self.train_P_B_cc_embed, \
            *self._get_batch(diseases))

        # concatenate drugs, diseases and output to num_classes
        ...
        if self.multilabel:  # this defaults to False
    ...
    def val_test_step(self, batch, batch_idx, is_test = False):
        ...
        # forward pass x2
        drugs, diseases = batch["drugs"], batch["diseases"]
        features_drugs = self.forward(mode, self.train_N_I_cc_embed, self.train_N_B_cc_embed, \
            self.train_S_I_cc_embed, self.train_S_B_cc_embed, self.train_P_I_cc_embed, self.train_P_B_cc_embed, \
            *self._get_batch(drugs))
        features_diseases = self.forward(mode, self.train_N_I_cc_embed, self.train_N_B_cc_embed, \
            self.train_S_I_cc_embed, self.train_S_B_cc_embed, self.train_P_I_cc_embed, self.train_P_B_cc_embed, \
            *self._get_batch(diseases))
...
# validation & test epoch end
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.hparams["resample_anchor_patches"]:
            if self.hparams["use_neighborhood"]:
                self.anchors_neigh_int, self.anchors_neigh_border = init_anchors_neighborhood("train_val", self.hparams, self.networkx_graph, self.device_num, self.cc_ids, self.cc_ids, self.cc_ids)
            if self.hparams["use_position"]:
                self.anchors_pos_int = init_anchors_pos_int("train_val", self.hparams, self.networkx_graph, self.device_num, self.subgraphs)
                self.anchors_pos_ext = init_anchors_pos_ext(self.hparams, self.networkx_graph, self.device_num)
            if self.hparams["use_structure"]:
...
# Read in data
    ...
    def read_data(self):
        ...
        # read in list of node ids for each subgraph
        self.subgraphs = subgraph_utils.read_subgraphs(config.PROJECT_ROOT / self.subgraph_path)

        # >>>
        # read in list of labels for subgraph pairs
        labels, drugs, diseases = subgraph_utils.read_subgraph_labels(config.PROJECT_ROOT / "covid_pairs/labels.txt")
        self.labels = labels
        self.drugs = drugs
        ...
        self.subgraphs = self.reindex_data(self.subgraphs)
        ...
        self.node_embeddings = nn.Embedding.from_pretrained(embeds, freeze=self.hparams["freeze_node_embeds"], padding_idx=config.PAD_VALUE).to(self.device_num)

        # NOTE: embeddings are [21585, 64] = fits in memory
        # check further downstream for out-of-memory bug
...
# Initialize connected components & associated embeddings for each channel in SubGNN
    ...
    def initialize_cc_embeddings(self, cc_id_list, aggregator="sum",
        ...
        """
        # preliminaries
        n_subg, max_n_cc, max_len_cc = cc_id_list.shape
        if aggregator == "sum":
            f_agg = lambda x,d: torch.sum(x, dim=d)
        elif aggregator == "max":
            f_agg = lambda x,d: torch.max(x, dim=d)[0]
        # cc_id_list is [2280, 157, 359] = [n subgraphs, max n cc, max len cc]
        # the 2280 = n_subgraphs dimension may be big, so instead of computing this all at once,
    ...
    def init_all_embeddings(self, split = "all", trainable = False):
        ...
        if split in ["all","train_val","train"]:
            ...
            train_cc_embeddings = self.initialize_cc_embeddings(self.cc_ids, self.hparams["cc_aggregator"])
        ...
        if split in ["all","train_val","val"]:
            val_cc_embeddings = self.initialize_cc_embeddings( self.cc_ids, self.hparams["cc_aggregator"])
        ...
        if split in ["all","test"]:
            test_cc_embeddings = self.initialize_cc_embeddings( self.cc_ids, self.hparams["cc_aggregator"])
        ...
        if self.hparams["use_neighborhood"] or (self.hparams["use_structure"] and self.hparams["structure_similarity_fn"] == "edit_distance"):
            ...
            if split == "test":
                if test_neigh_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.test_N_border = self.initialize_border_sets(test_neigh_path, self.cc_ids, self.hparams["neigh_sample_border_size"], ego_graph_dict)
                ...
                if train_neigh_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.train_N_border = self.initialize_border_sets(train_neigh_path, self.cc_ids,  self.hparams["neigh_sample_border_size"], ego_graph_dict)
                if val_neigh_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.val_N_border = self.initialize_border_sets(val_neigh_path, self.cc_ids, self.hparams["neigh_sample_border_size"], ego_graph_dict)
    ...
    def get_similarities(self, split):
        ...
        if self.hparams["use_position"] or self.hparams["use_neighborhood"]:
            ...
            if split == "test":
                if test_np_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.test_neigh_pos_similarities = torch.tensor(np.load(test_np_path, allow_pickle=True))#.to(self.device_num)
                else:
                    self.test_neigh_pos_similarities = self.compute_shortest_path_similarities(test_np_path, pairwise_shortest_paths, self.cc_ids)
            elif split == "train_val":
                if train_np_path.exists() and not self.hparams["compute_similarities"]:
                    print("--- Loading Train Position Similarities from File ---")
                    self.train_neigh_pos_similarities = torch.tensor(np.load(train_np_path, allow_pickle=True))#.to(self.device_num)
                else:
                    self.train_neigh_pos_similarities = self.compute_shortest_path_similarities(train_np_path, pairwise_shortest_paths, self.cc_ids)

                if val_np_path.exists() and not self.hparams["compute_similarities"]:
                    print("--- Loading Val Position Similarities from File ---")
                    self.val_neigh_pos_similarities = torch.tensor(np.load(val_np_path, allow_pickle=True))#.to(self.device_num)
                else:
                    self.val_neigh_pos_similarities = self.compute_shortest_path_similarities(val_np_path, pairwise_shortest_paths, self.cc_ids)
        ...
        if self.hparams["use_structure"]:
            ...
            if struc_anchor_patches_path.exists() and not self.hparams["compute_similarities"]:
                ...
                self.structure_anchors = sample_structure_anchor_patches(self.hparams, self.networkx_graph, self.device_num, self.hparams["max_sim_epochs"])
            ...
            if bor_struc_patch_random_walks_path.exists() and not self.hparams["compute_similarities"]:
                self.bor_structure_anchor_random_walks = torch.tensor(np.load(bor_struc_patch_random_walks_path, allow_pickle=True))#.to(self.device_num)
            else:
            ...
            if int_struc_patch_random_walks_path.exists() and not self.hparams["compute_similarities"]:
                self.int_structure_anchor_random_walks = torch.tensor(np.load(int_struc_patch_random_walks_path, allow_pickle=True))#.to(self.device_num)
            else:
            ...
            if split == "test":
                if test_int_struc_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.test_int_struc_similarities = torch.tensor(np.load(test_int_struc_path, allow_pickle=True))#.to(self.device_num)
                else:
                    self.test_int_struc_similarities = self.compute_structure_patch_similarities(degree_dict, test_int_struc_path, True, self.cc_ids, sim_path, "test", self.test_N_border)
                ...
                if train_int_struc_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.train_int_struc_similarities = torch.tensor(np.load(train_int_struc_path, allow_pickle=True))#.to(self.device_num)
                else:
                    self.train_int_struc_similarities = self.compute_structure_patch_similarities(degree_dict, train_int_struc_path, True, self.cc_ids, sim_path, "train", self.train_N_border)
                ...
                if val_int_struc_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.val_int_struc_similarities = torch.tensor(np.load(val_int_struc_path, allow_pickle=True))#.to(self.device_num)
                else:
                    self.val_int_struc_similarities = self.compute_structure_patch_similarities(degree_dict, val_int_struc_path, True, self.cc_ids, sim_path, "val", self.val_N_border)
            ...
            if split == "test":
                if test_bor_struc_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.test_bor_struc_similarities = self.compute_structure_patch_similarities(degree_dict, test_bor_struc_path, False, self.cc_ids, sim_path, "test", self.test_N_border)
            ...
            if split == "train_val":
                if train_bor_struc_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.train_bor_struc_similarities = self.compute_structure_patch_similarities(degree_dict, train_bor_struc_path, False, self.cc_ids, sim_path,"train", self.train_N_border)
                ...
                if val_bor_struc_path.exists() and not self.hparams["compute_similarities"]:
                    ...
                    self.val_bor_struc_similarities = self.compute_structure_patch_similarities(degree_dict, val_bor_struc_path, False, self.cc_ids, sim_path, "val", self.val_N_border)
...
# Prepare data
    ...
    def prepare_test_data(self):
        ...
        # TODO
        self.cc_ids = self.initialize_cc_ids(self.subgraphs)
        ...
        if self.hparams["use_neighborhood"]:
            self.anchors_neigh_int, self.anchors_neigh_border = init_anchors_neighborhood("test", \
                 self.hparams, self.networkx_graph, self.device_num, None, None, \
                     self.cc_ids, None, None, self.test_N_border)
        else: self.anchors_neigh_int, self.anchors_neigh_border = None, None
        if self.hparams["use_position"]:
            self.anchors_pos_int = init_anchors_pos_int("test", self.hparams, self.networkx_graph, self.device_num, self.subgraphs)
    ...
    def prepare_data(self):
        ...
        if self.hparams["use_neighborhood"]:
            self.anchors_neigh_int, self.anchors_neigh_border = init_anchors_neighborhood("train_val", \
                 self.hparams, self.networkx_graph, self.device_num, self.cc_ids, self.cc_ids, \
...
# Data loaders
    ...
    def _pad_collate(self, batch):
        ...
        """
        # wrapper around original _pad_collate, renamed _pad_single
        # because our inputs are now pairs of subgraphs
        drugs, diseases, labels = zip(*batch)
        # transform subgraphs
        drugs = self._pad_single(drugs)
        diseases = self._pad_single(diseases)
    ...
    def _pad_single(self, subgraphs):
        """
            Original _pad_collate
        """
        subgraph_ids, con_component_ids, N_border, NP_sim, I_S_sim, B_S_sim, idx = zip(*subgraphs)
    ...
    def train_dataloader(self):
        ...
        # raw subgraph dataset
        dataset = SubgraphDataset(self.subgraphs, self.cc_ids, \
            self.train_N_border, self.train_neigh_pos_similarities, self.train_int_struc_similarities, \
                self.train_bor_struc_similarities)

        # >>>
        # pairs of subgraphs dataset
        dataset = SubgraphPairDataset(dataset, self.labels["train"],
                                      self.drugs, self.diseases)

        # drop last examples in batch if batch size is <= number of subgraphs in the training set (this will usually evaluate to true)
        drop_last = self.hparams["batch_size"] <= len(self.subgraphs)
        loader = DataLoader(dataset,
                            batch_size=self.hparams["batch_size"],
                            num_workers=self.hparams["n_dataloader_workers"],
                            shuffle=True, collate_fn=self._pad_collate, drop_last=drop_last)
        return loader
    ...
    def val_dataloader(self):
        ...
        dataset = SubgraphDataset(self.subgraphs, self.cc_ids, \
            self.val_N_border, self.val_neigh_pos_similarities, self.val_int_struc_similarities, \
                self.val_bor_struc_similarities)

        # >>>
        # pairs of subgraphs dataset
        dataset = SubgraphPairDataset(dataset, self.labels["val"],
                                      self.drugs, self.diseases)
        loader = DataLoader(dataset, batch_size = self.hparams["batch_size"],
                            num_workers=self.hparams["n_dataloader_workers"],
                            shuffle=False, collate_fn=self._pad_collate)
        return loader
    ...
    def test_dataloader(self):
        ...
        dataset = SubgraphDataset(self.subgraphs, self.cc_ids, \
            self.test_N_border, self.test_neigh_pos_similarities, self.test_int_struc_similarities, \
                self.test_bor_struc_similarities)

        # >>>
        # pairs of subgraphs dataset
        # NOTE do not sample negatives for test
        dataset = SubgraphPairDataset(dataset, self.labels["test"],
                                      self.drugs, self.diseases,
